...
df_outbreak = pd.read_pickle(
    os.path.join(subfolder,  "Outbreaks", "Outbreak_Chain 1_10_0.pkl")
)
df_stores = pd.read_pickle(os.path.join(subfolder, "Stores", "stores.pkl"))
df_population = pd.read_pickle(
    os.path.join(subfolder, "Population", "population.pkl")
)

# Create Points for stores and outbreak
df_stores["geometry"] = gpd.points_from_xy(df_stores["x"], df_stores["y"])
...
df_population["geometry"] = df_population.apply(
    lambda row: Polygon(
        [
            (row["x_centroid"] - 0.05, row["y_centroid"] - 0.05),
            (row["x_centroid"] - 0.05, row["y_centroid"] + 0.05),
            (row["x_centroid"] + 0.05, row["y_centroid"] + 0.05),
            (row["x_centroid"] + 0.05, row["y_centroid"] - 0.05),
        ]
